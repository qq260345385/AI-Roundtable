...
1. 输入一个你想讨论的议题。
2. 确认默认选中的参会模型；需要时再展开“模型选择”调整。
3. 可选：展开“更多设置”，导入本地资料或开启 Tavily 联网搜索。
4. 开始会议，等待模型完成三阶段讨论。
5. 先阅读决策简报中的推荐结论、置信度、成立条件和下一步行动，再按需展开完整过程与依据。
...
会议完成后，结果页先显示结构化决策简报：推荐结论、状态与置信度、核心理由、成立条件、主要反对意见、缺失证据、推翻条件和一个近期行动。低证据、无效引用、严重分歧、模型失败或参会模型不足会自动降低建议强度。

“详细过程与依据”默认折叠；展开后可查看共识、分歧、阶段复盘、模型对照、证据/引用状态和失败记录，也可以用阶段切换器回看原始发言。
...
`npm run test:e2e` 运行桌面端快速会议与降级会议流程，并保存完整全页截图；它独立于默认 `verify`，CI 会单独运行。
...
- `v0.8.0`：加入保守降级的决策简报、结论优先结果/导出、聚焦式桌面首页和桌面全流程测试。
