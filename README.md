...
联网搜索会在服务端做本地内存缓存、URL 规范化、重复 URL 合并、同域名数量限制和 Evidence Rescue。Evidence Rescue 会先保留广搜候选池，在可用资料不足时对少量 URL 做 Tavily Extract 正文抽取，再重新评分并精选入包。普通用户只需要选择普通搜索或深度搜索；候选池、抽取次数、缓存命中、去重原因和评分拆解都属于后台优化，默认 UI 不展示。完整统计仅在本地开发调试模式的 `debugSearchProcess` 和 `npm run test:live-search` 输出中可见。本地内存缓存会在服务重启后清空。
